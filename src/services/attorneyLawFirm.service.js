...
const getLawFirms = async (query) => {
  ...
  const where = query.search
    ...
    : {};
  const { lawFirms, total } = await attorneyLawFirmRepository.getLawFirms({ skip, take: limit, where });
...
const createAttorney = async (data, currentUser) => {
  ...
  return attorneyLawFirmRepository.createAttorneyWithRepresentation(attorneyData, {
...
const mapAttorneyForCase = async (attorney, caseId) => ({
  ...attorney,
  representation: await attorneyLawFirmRepository.findRepresentation(attorney.id, caseId),
});
...
const getAttorneys = async (query, currentUser) => {
  ...
  const { attorneys, total } = await attorneyLawFirmRepository.getAttorneys({ skip, take: limit, where });
  return {
    attorneys: await Promise.all(attorneys.map((attorney) => mapAttorneyForCase(attorney, caseId))),
...
const getAttorneyById = async (id, caseId, currentUser) => {
  ...
  const representation = await attorneyLawFirmRepository.findRepresentation(id, caseId);
  if (!representation) throw new ApiError(404, "Attorney is not linked to this case.");
...
const updateAttorney = async (id, data, currentUser) => {
  ...
  await Promise.all([...new Set(caseIds.map((item) => item.caseParty.caseId))].map((caseId) => caseService.getCaseById(caseId, currentUser)));
