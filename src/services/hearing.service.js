  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      await recordEvent(tx, {
        ...
        summary: `${hearing.title} scheduled.`,
        actorUserId: currentUser.id,
        newValue: { startTime: hearing.startTime, endTime: hearing.endTime },
      });
      return hearing;
    },
    { timeout: 15000 },
...
const getHearing = (caseId, hearingId, currentUser) =>
  getAuthorizedHearing(caseId, hearingId, currentUser);
...
const rescheduleHearing = async (caseId, hearingId, data, currentUser) => {
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      const updated = await hearingRepository.updateHearing(
        hearingId,
        {
          hearingDate: new Date(data.startTime),
          startTime: new Date(data.startTime),
      ...
      await recordEvent(tx, {
        ...
        summary: `${updated.title} rescheduled.`,
        actorUserId: currentUser.id,
        previousValue: {
          startTime: hearing.startTime,
          endTime: hearing.endTime,
...
const cancelHearing = async (caseId, hearingId, reason, currentUser) => {
  ...
  return prisma.$transaction(
    async (tx) => {
      const updated = await hearingRepository.updateHearing(
        hearingId,
        { hearingStatus: "CANCELLED", cancelReason: reason },
        tx,
      );
      await recordEvent(tx, {
        caseId,
        hearingId,
        eventType: "STATUS_CHANGED",
        summary: `${updated.title} cancelled.`,
        actorUserId: currentUser.id,
