...
const createParty = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(
    new ApiResponse(201, result, "Party added successfully.")
  );
});
...
const updateParty = asyncHandler(async (req, res) => {
  const result = await partyService.updateParty(req.params.id, req.body, req.user);

  return res.status(200).json(
    new ApiResponse(200, result, "Party updated successfully.")
  );
});
